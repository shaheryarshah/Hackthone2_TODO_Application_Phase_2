...
export default function Home() {
  ...
  const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://hackthone-todo-pii-back-9x7ovdbdd-shaheryars-projects-3e695ce4.vercel.app/api/v1';
  ...
  const fetchTodos = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/todos?${params.toString()}`, { headers });
      if (!response.ok) {
  ...
  }, [filters, sort]); // Run when filters or sort change
